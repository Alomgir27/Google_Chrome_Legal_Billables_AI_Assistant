...
class ClioService {
  ...
  getAuthUrl(platform) {
    ...
    const redirectUri = 'http://127.0.0.1:3000/auth/clio/callback'; // Backend callback URL
  ...
  async exchangeCodeForToken(authCode) {
    try {
      ...
      const tokenData = new URLSearchParams({
        ...
        code: authCode,
        redirect_uri: 'http://127.0.0.1:3000/auth/clio/callback'
      });
  ...
  getAuthUrl() {
    ...
    const redirectUri = 'http://127.0.0.1:3000/auth/clio/callback';
    const scope = 'read write';
    const state = Math.random().toString(36).substring(2, 15);
